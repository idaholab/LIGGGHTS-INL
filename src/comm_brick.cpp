...
CommBrick::CommBrick(LAMMPS *lmp) :
  ...
  sendlist(NULL), maxsendlist(NULL)
{
  style = 0;
...
{
  if (oldcomm->get_layout() == LAYOUT_TILED)
    error->all(FLERR,"Cannot change to comm_style brick from tiled layout");

  style = 0;
  layout = oldcomm->get_layout();
  Comm::copy_arrays(oldcomm);
  init_buffers();
}

CommBrick::CommBrick(Comm *main_comm, ParallelBase *pb) :
  Comm(*main_comm)
{
  style = 0;
  is_subcomm = true;
  pb_ = pb;
  main_comm->register_subcomm(this);
  Comm::copy_pointers(main_comm);
  init_buffers();
  subcomms_.clear();
}
...
void CommBrick::setup()
{
  ...
  for (dim = 0; dim < 3; dim++) {
    ...
    if (pb_)
      slab_offset = pb_->get_slab_offset(dim);
    else
      slab_offset = use_gran_opt() ? (cutghost[dim] / 2. + neighbor->skin/2.) : cutghost[dim];
    for (ineed = 0; ineed < 2*maxneed[dim]; ineed++) {
      ...
      if (ineed % 2 == 0) {
        ...
        if (mode == SINGLE) {
          ...
          slabhi[iswap] = sublo[dim] + slab_offset; 
        } else {
          for (int i = 1; i <= atom->ntypes; i++) {
        ...
        if (style == SINGLE) {
          slablo[iswap] = subhi[dim] - slab_offset; 
...
void CommBrick::forward_comm(int dummy)
{
  ...
  double nrecv_scale[2] = {0., 0.};
  if (pb_) {
    pb_->get_nrecv_scale(OPERATION_COMM_FORWARD, nrecv_scale);
  ...
  for (int iswap = 0; iswap < nswap; iswap++) {
    if (sendproc[iswap] != me) {
      if (pb_) {
        const int size_recv = (size_forward_recv[iswap]*nrecv_scale[0])/nrecv_scale[1];
        if (size_recv)
          MPI_Irecv(buf_recv,size_recv,MPI_DOUBLE,
                    recvproc[iswap],0,world,&request);
        const int n = pb_->pack_comm(OPERATION_COMM_FORWARD, sendnum[iswap], sendlist[iswap],
                                     buf_send, pbc_flag[iswap], pbc[iswap]);
        if (n) MPI_Send(buf_send,n,MPI_DOUBLE,sendproc[iswap],0,world);
        if (size_recv) MPI_Wait(&request,MPI_STATUS_IGNORE);
        pb_->unpack_comm(OPERATION_COMM_FORWARD, recvnum[iswap], firstrecv[iswap], buf_recv);
      } else if (comm_x_only) {
        ...
        const int n = avec->pack_comm(sendnum[iswap],sendlist[iswap],
                                      buf_send,pbc_flag[iswap],pbc[iswap]);
        if (n) MPI_Send(buf_send,n,MPI_DOUBLE,sendproc[iswap],0,world);
        ...
        if (size_forward_recv[iswap])
          MPI_Irecv(buf_recv,size_forward_recv[iswap],MPI_DOUBLE,
                    recvproc[iswap],0,world,&request);
        const int n = avec->pack_comm_vel(sendnum[iswap],sendlist[iswap],
                                          buf_send,pbc_flag[iswap],pbc[iswap]);
        if (n) MPI_Send(buf_send,n,MPI_DOUBLE,sendproc[iswap],0,world);
        if (size_forward_recv[iswap]) MPI_Wait(&request,MPI_STATUS_IGNORE);
      ...
      if (pb_) {
        pb_->pack_comm(OPERATION_COMM_FORWARD, sendnum[iswap],sendlist[iswap],
                       buf_send,pbc_flag[iswap],pbc[iswap]);
        pb_->unpack_comm(OPERATION_COMM_FORWARD, recvnum[iswap], firstrecv[iswap], buf_send);
      } else if (comm_x_only) {
        if (sendnum[iswap])
...
void CommBrick::reverse_comm()
{
  ...
  double nrecv_scale[2] = {0., 0.};
  if (pb_) {
    pb_->get_nrecv_scale(OPERATION_COMM_REVERSE, nrecv_scale);
  ...
  for (int iswap = nswap-1; iswap >= 0; iswap--) {
    if (sendproc[iswap] != me) {
      if (pb_) {
        const int size_recv = (size_reverse_recv[iswap]*nrecv_scale[0])/nrecv_scale[1];
        if (size_recv)
          MPI_Irecv(buf_recv,size_recv,MPI_DOUBLE,
                    sendproc[iswap],0,world,&request);
        n = pb_->pack_reverse(OPERATION_COMM_REVERSE, recvnum[iswap],firstrecv[iswap],buf_send);
        if (n) MPI_Send(buf_send,n,MPI_DOUBLE,recvproc[iswap],0,world);
        if (size_recv) MPI_Wait(&request,MPI_STATUS_IGNORE);
        pb_->unpack_reverse(OPERATION_COMM_REVERSE, sendnum[iswap],sendlist[iswap],buf_recv);
      } else if (comm_f_only) {
      ...
      if (pb_) {
        pb_->pack_reverse(OPERATION_COMM_REVERSE, recvnum[iswap],firstrecv[iswap],buf_send);
        pb_->unpack_reverse(OPERATION_COMM_REVERSE, sendnum[iswap],sendlist[iswap],buf_send);
      } else if (comm_f_only) {
        if (sendnum[iswap])
...
{
  int i,n,iswap,dim,ineed,twoneed;
  int nsend,nrecv,sendflag,nfirst,nlast,ngroup;
  double lo = 0.0,hi = 0.0;
  double **x;
  double *buf, *mlo = NULL, *mhi = NULL;
  MPI_Request request;
  AtomVec *avec = atom->avec;
    ...
    for (ineed = 0; ineed < twoneed; ineed++) {
      ...
      if (sendflag) {
        if (!bordergroup || ineed >= 2) {
          if (bordergroup && pb_)
            error->all(FLERR, "Group command not implemented for ParallelBase");
          if (mode == SINGLE) {
      ...
      if (pb_)
        n = pb_->pack_comm(OPERATION_COMM_BORDERS,
                           nsend,sendlist[iswap],buf_send,
      ...
      if (pb_)
        pb_->unpack_comm(OPERATION_COMM_BORDERS,
                         nrecv, pb_->get_ntotal(), buf);
      else {
        if (ghost_velocity)
      ...
      firstrecv[iswap] = pb_ ? pb_->get_ntotal() : atom->nlocal + atom->nghost;
      if (pb_)
        pb_->add_nghost(nrecv);
      else
        atom->nghost += nrecv;
      iswap++;
    }
  ...
  if (pb_)
    pb_->generate_map();
  else if (map_style) atom->map_set();
...
void CommBrick::forward_comm_pair(Pair *pair)
{
  ...
  for (iswap = 0; iswap < nswap; iswap++) {
    ...
    if (sendproc[iswap] != me) {
      if (recvnum[iswap])
        MPI_Irecv(buf_recv,n*recvnum[iswap],MPI_DOUBLE,
                  recvproc[iswap],0,world,&request);
      if (sendnum[iswap])
...
inline bool CommBrick::use_gran_opt()
{
    const bool has_radius = pb_ ? pb_->has_radius() : atom->radius != NULL;
    return (domain->triclinic == 0 && has_radius);
}
...
inline bool CommBrick::decide(int i,int dim,double lo,double hi,int ineed)
{
    double **x = atom->x;
    const double pos = pb_ ? pb_->get_pos(i,dim) : x[i][dim];
    const double radius = pb_ ? pb_->get_radius(i) : atom->radius[i];
    const double offset = use_gran_opt() ? radius : 0.;

    if( ((ineed % 2 == 0) && pos >= lo && pos <= (hi + offset) ) ||
        ((ineed % 2 == 1) && pos >= (lo - offset) && pos <= hi )   )
        return true;
...
inline bool CommBrick::decide_wedge(int i,int dim,double lo,double hi,int ineed)
{
    double **x = atom->x;
    double coo[2],d[2];
    coo[0] = pb_ ? pb_->get_pos(i, iphi) : x[i][iphi];
    coo[1] = pb_ ? pb_->get_pos(i, (iphi+1)%3) : x[i][(iphi+1)%3];
    const double radius = pb_ ? pb_->get_radius(i) : atom->radius[i];
    const double offset = use_gran_opt() ? radius : 0.;
    ...
    else if (ineed % 2 == 1)
    {
        ...
        if(vectorDot2D(d,nright) >= -offset)
        {
...
/* ----------------------------------------------------------------------
   ...
   similar to CommBrick:Exchange()
------------------------------------------------------------------------- */

void CommBrick::exchangeEventsRecorder()
{
    ...
    //since called in CommBrick::Exchange, triclinic boundaries are already correct
    if (domain->triclinic == 0) {
      sublo = domain->sublo;
      subhi = domain->subhi;
    } else {
    ...
    if(verbose)
       printf("[%d/%d]:CommBrick::exchangeEventsRecorder(); lo/hi: %g %g %g / %g %g %g, exchanging sublo/hi information with process %d %d %d: %g %g %g / %g %g %g \n",
               me, nprocs,
...
//        if(verbose)
//           printf("[%d/%d]:CommBrick::exchangeEventsRecorder() looping  atom %d with pos: %g %g %g\n",
//                       me, nprocs, i, x[i][0], x[i][1], x[i][2]);
        ...
        for (int dim = 0; dim < 3; dim++)
        {
            ...
            if(verbose)
              printf("[%d/%d]:CommBrick::exchangeEventsRecorder() looping dim: %d, atom %d with pos: %g\n",
            ...
            if (position < lo || position >= hi)
            {
                ...
                if(verbose)
                printf("[%d/%d]:CommBrick::exchangeEventsRecorder() recording exchange for atom %d with pos: %g\n",
...
void CommBrick::exchangeEventsCorrector()
{
    if( !exchangeEvents || nprocs==1 )
        return;
    ...
    for(int j=0;j<global_sum;j++)
    {
       ...
       if(exchangeGlobalProblems[j]<0)
            error->one(FLERR,"CommBrick::exchangeEventsCorrector: Could not find particle. Must have left the domain. You must upgrade this check in order to handle this situation.");
    }
    ...
    if( checkCounter!=exchangeEventsGlobalProblemIds.size() )
        error->all(FLERR,"CommBrick::exchangeEventsCorrector: Problem when fill in corrected process ids! This is fatal.");
