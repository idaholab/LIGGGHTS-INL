...
TANGENTIAL_MODEL(TANGENTIAL_HYSTERETIC_NONLINEAR,hysteretic/nonlinear,5)
#else
#ifndef TANGENTIAL_MODEL_HYSTERETIC_NONLINEAR_H_
#define TANGENTIAL_MODEL_HYSTERETIC_NONLINEAR_H_
...
{
  template<>
  class TangentialModel<TANGENTIAL_HYSTERETIC_NONLINEAR> : public TangentialModelBase
  {
  public:
        ...
        torsion_offset_(-1)
    {
        ...
        hsetup->add_history_value("shrmag_0","1");//history[3]
        hsetup->add_history_value("sh_0","1");
        hsetup->add_history_value("sh_1","1");
        hsetup->add_history_value("sh_2","1");
    }
    ...
    void connectToProperties(PropertyRegistry & registry)
    {
        ...
        registry.connect("coeffFrict", coeffFrict,"tangential_model hysteretic/nonlinear");
    ...
    void surfacesIntersect(const SurfacesIntersectData & sidata, ForceData & i_forces, ForceData & j_forces)
    {
        ...
        // plastic range
        // read historic tangential overlap
        double shrmag_0 = history[3];
        double sh_0 = history[4];
        double sh_1 = history[5];
        double sh_2 = history[6];
        if (deltan <= deltaZero)
        {
           shrmag_0 = shrmag;
           history[3] = shrmag_0;
        ...
        //std::cout<< "id_i " << id_i << " id_j " << id_j << " shrmag " << shrmag << " Ft_shear "<<Ft_shear<<" sidata.Fn "<<sidata.Fn<<" Ft_friction "<<Ft_friction;
        int tag_status = 0;

        // energy loss from sliding or damping
        if (Ft_shear > Ft_friction)
        {   //F_tangential = sliding friction
            tag_status = 1;
            if (shrmag != 0.0)
            {
        ...
        else //Ft_shear < Ft_friction
        {   //Ft_shear = shear force
            tag_status = 2;
        ...
        // cout tangential force
        //std::cout << std::scientific;
        //std::cout <<"d0 "<<sidata.deltaZero<<" "<<tag_status<< " ift1 " << i_forces.delta_F[0] << " ift2 " << i_forces.delta_F[1]  << " ift3 " << i_forces.delta_F[2] << std::endl;
        //std::cout << "itor1 " << i_forces.delta_torque[0] << " itor2 " << i_forces.delta_torque[1] << " itor3 " << i_forces.delta_torque[2] << std::endl;
        //std::cout<<" ift1 " << i_forces.delta_F[0] << " ift2 " << i_forces.delta_F[1] << " ift3 " << i_forces.delta_F[2] << std::endl;
    }
...
#endif // TANGENTIAL_MODEL_HYSTERETIC_NONLINEAR_H_
#endif
