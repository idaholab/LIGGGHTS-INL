    ...
    heatPhy(lmp)
{
    ...
    {
        int64_t variant = Factory::instance().selectVariant("gran", nremaining, remaining_args, lmp);
        if (variant == -1)
    ...
    while(iarg_ < narg && hasargs)
    {
        ...
        else if (strcmp(arg[iarg_],"shear") == 0)
        {
            ...
            if (strcmp(arg[iarg_+1],"x") == 0)
                shearDim_ = 0;
            else if (strcmp(arg[iarg_+1],"y") == 0)
                shearDim_ = 1;
            else if (strcmp(arg[iarg_+1],"z") == 0)
                shearDim_ = 2;
            else
                error->fix_error(FLERR,this,"illegal 'shear' dim");
    ...
    // parse heat transfer arguments
    heatPhy.parseArgs(iarg_,narg,arg);
...
void FixWallGran::post_create()
{
    ...
    // avoid find_fix_style in constructor; this may cause problems during replacing existing fixes
...
{
    if(primitiveWall_ != 0)
        delete primitiveWall_;
    if(FixMesh_list_)
        delete []FixMesh_list_;
    delete impl;
}
        ...
        for(int iTri = 0; iTri < nTriAll; iTri++)
        {
            ...
            for(int iCont = 0; iCont < numneigh; iCont++)
            {
                ...
                if(atom->shapetype_flag)
                {
                    ...
                    sidata.j = iTri;
                    fix_contact->handleContact(iPart,idTri,sidata.contact_history,intersectflag,false,false);
                    if(vMeshC)
                    {
                        for(int i = 0; i < 3; i++)
                ...
                if(deltan <= 0 || (radius && deltan < contactDistanceMultiplier*radius[iPart]))
                {
                    ...
                    if(!atom->shapetype_flag && fix_contact && ! fix_contact->handleContact(iPart,idTri,sidata.contact_history,intersectflag,7 == barysign, mesh->getWeightedWallFormulation()))
                        continue;
...
int FixWallGran::n_neighs_local() const
{
    if (!is_mesh_wall())
        return 0;
    ...
    for (int i = 0; i < n_FixMesh_; ++i)
        for (int j = 0; j < atom->nlocal; ++j)
            ncontacts += FixMesh_list_[i]->contactHistory()->nneighs(j);
...
void FixWallGran::addHeatFlux(TriMesh *mesh,SurfacesIntersectData & sidata,int ip, const double ri, double delta_n, double area_ratio)
{
    ...
    double contactArea; // dummy
    const double hc = heatPhy.computeHtPropertiesPW(ip,delta_n,sidata.contact_history,atom_type_wall_,Temp_wall,contactArea);
    ...
    call_add_heat_pw_callback(sidata,(Temp_wall-Temp_p[ip]) * hc);
...
int FixWallGran::modify_param(int narg, char **arg)
{
    if (strcmp(arg[0],"temperature") == 0)
    {
        if (narg < 2)
          error->fix_error(FLERR,this,"not enough arguments for 'temperature'");
        Temp_wall = force->numeric(FLERR,arg[1]);
        return 2;
    }
