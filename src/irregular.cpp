...
------------------------------------------------------------------------- */
void Irregular::migrate_all(const bool setupflag, int sortflag, int preassign, int **procassign)
{
  migrate_atoms(sortflag, preassign, preassign ? procassign[pb_.size()] : NULL);
  std::list<ParallelBase*>::iterator it;
  int i;
  for (it = pb_.begin(), i = 0; it != pb_.end(); it++, i++)
    migrate_pb(*it, sortflag, preassign, preassign ? procassign[i] : NULL);
}
...
void Irregular::migrate_pb(ParallelBase *pb, int sortflag, int preassign, int *procassign)
{
  if (!pb->do_migrate())
    return;
  // Clear all ghosts and map, equivalent to what happens in Comm::exchange()
  pb->clear_exchange();
  ...
  // if Comm will be called to assign new atom coords to procs,
  // may need to setup RCB info
  // No need to call mycomm_ of the respective pb implementations
  // as the two comm functions used in this function are identical
  ...
  if (preassign) {
    while (i < nlocal) {
      if (procassign[i] == me) i++;
        ...
        mproclist[nsendatom] = procassign[i];
        if (nsend > maxsend) grow_send(nsend,1);
        msizes[nsendatom] = pb->pack_irregular(i,&buf_send[nsend]);
        nsend += msizes[nsendatom];
        nsendatom++;
        procassign[i] = procassign[nlocal-1];
        nlocal--;
    ...
    while (i < nlocal) {
      const double x[3] = {pb->get_pos(i, 0),
                           pb->get_pos(i, 1),
                           pb->get_pos(i, 2)};
      if (x[0] < sublo[0] || x[0] >= subhi[0] ||
          x[1] < sublo[1] || x[1] >= subhi[1] ||
          x[2] < sublo[2] || x[2] >= subhi[2]) {
        mproclist[nsendatom] = comm->coord2proc(x,igx,igy,igz);
        if (mproclist[nsendatom] == me) i++;
        else {
          if (nsend > maxsend) grow_send(nsend,1);
  ...
  // create irregular communication plan, perform comm, destroy plan
  // returned nrecv = size of buffer needed for incoming atoms
  // For pb we just reuse what is already here as it is independent of
  ...
  int m = 0;
  while (m < nrecv) m += pb->unpack_irregular(&buf_recv[m]);
...
int Irregular::migrate_check()
{
  ...
  if (comm->get_layout() == LAYOUT_TILED) return 1;

  const int mca = migrate_check_atom();
  if (mca)
    return 1;

  std::list<ParallelBase*>::iterator it;
  for (it = pb_.begin(); it != pb_.end(); it++)
  {
    const int mcpb = migrate_check_pb(*it);
    if (mcpb)
      return mcpb;
  }
  return 0;
...
int Irregular::migrate_check_atom()
{
  // subbox bounds for orthogonal or triclinic box
...
int Irregular::migrate_check_pb(ParallelBase* pb)
{
  // subbox bounds for orthogonal or triclinic box
  ...
  // loop over elements, check for any that are not in my sub-box
  // assign which proc it belongs to via Comm::coord2proc()
  // if logical igx,igy,igz of newproc > one away from myloc, set flag = 1
  // this check needs to observe PBC
  // cannot check via comm->procneigh since it ignores PBC

  const int nlocal = pb->get_nlocal();
