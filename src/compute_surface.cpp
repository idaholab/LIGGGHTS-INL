    ...
    cosine_sqr_(0.)
{
    ...
    if (narg < iarg)
        error->compute_error(FLERR,this,"not enoguh arguments");
    ...
    while(iarg < narg && hasargs)
    {
        ...
        else if (strcmp(arg[iarg],"region") == 0)
        {
            ...
            iarg++;
            iregion_ = domain->find_region(arg[iarg++]);
            if (iregion_ == -1)
                error->compute_error(FLERR,this,"Group region ID does not exist");
        }
...
void ComputeSurface::compute_peratom()
{
    ...
    if (iregion_ != -1)
    {
        const double masstotal = group->mass(igroup, iregion_);
        group->xcm(igroup, masstotal, com, iregion_);
    }
    else
    {
        const double masstotal = group->mass(igroup);
    ...
    Region *region = iregion_ != -1 ? domain->regions[iregion_] : NULL;
    for (i = 0; i < nall; i++)
    {
        vectorSubtract3D(x[i],com,com_to_xi);
        // particle on wrong side of "up" - might be "on surface"
        if(vectorDot3D(com_to_xi,use_com_ ? com_to_up_unit : n_vec_up_) > 0. && (iregion_ == -1 || region->match(x[i][0],x[i][1],x[i][2])))
            is_on_surface_[i] = 1.;
    ...
    for (ii = 0; ii < inum; ii++)
    {
        i = ilist[ii];
        if (mask[i] & groupbit &&
            (iregion_ == -1 || region->match(x[i][0],x[i][1],x[i][2])))
        {
