...
Comm::~Comm()
{
  ...
  // if we area subcomm we only have pointers to these adresses
  // in this case we do nothing and let the main comm destroy
  // everything else
  if (is_subcomm)
    return;
...
void Comm::copy_pointers(Comm *oldcomm)
{
    grid2proc = oldcomm->grid2proc;
    xsplit = oldcomm->xsplit;
    ysplit = oldcomm->ysplit;
    zsplit = oldcomm->zsplit;
    cutusermulti = oldcomm->cutusermulti;
    customfile = oldcomm->customfile;
    outfile = oldcomm->outfile;
}
...
void Comm::set(int narg, char **arg)
{
  ...
  if (strcmp(arg[0],"single") == 0 || strcmp(arg[0],"multi") == 0)
  {
    if (comm->me == 0)
        error->warning(FLERR, "Missing keyword mode in front of single or multi");
    if (strcmp(arg[0], "single") == 0)
        found_single = true;
    if (strcmp(arg[0], "multi") == 0)
        found_multi = true;
  }

  int iarg = 0;
  while (iarg < narg) {
    if (strcmp(arg[iarg],"mode") == 0 || found_multi || found_single) {
      int this_arg = (found_single || found_multi) ? 1 : 2;
      if (iarg+this_arg > narg) error->all(FLERR,"Illegal communicate command");
      if (strcmp(arg[iarg+1],"single") == 0 || found_single) {
        ...
        mode = SINGLE;
        found_single = false;
      } else if (strcmp(arg[iarg+1],"multi") == 0 || found_multi) {
        // need to reset cutghostuser when switching comm mode
...
void Comm::set_processors(int narg, char **arg)
{
  ...
  if (!p)
  {
      ...
      if (p && nprocs % p)
          error->all(FLERR, "Physical processors is not a multiple of specified processors");
  }
...
int Comm::coord2proc(const double *const x, int &igx, int &igy, int &igz)
{
  double *prd = domain->prd;
  double *boxlo = domain->boxlo;

  // initialize triclinic b/c coord2proc can be called before Comm::init()
  // via Irregular::migrate_all()
...
void Comm::grow_send(int n, int flag, const int requestextra)
{
  maxsend = static_cast<int> (BUFFACTOR * n);
  const int extra = requestextra > 0 ? requestextra : bufextra;
  if (flag)
    memory->grow(buf_send,maxsend+extra,"comm:buf_send");
  else {
    memory->destroy(buf_send);
    memory->create(buf_send,maxsend+extra,"comm:buf_send");
  }
}
...
/* ----------------------------------------------------------------------
   check whether growing of send buffer is required and if yes, do it
------------------------------------------------------------------------- */

double* Comm::check_grow_send(const int n, const int flag, const int requestextra)
{
    if (n > maxsend)
        grow_send(n, flag, requestextra);
    return buf_send;
}

void Comm::set_mysplit(const double _mysplit[3][2])
{
    memcpy(&mysplit[0][0], &_mysplit[0][0], 6*sizeof(double));
    if (!subcomms_.empty())
    {
        std::list<Comm*>::iterator it;
        for (it = subcomms_.begin(); it != subcomms_.end(); it++)
            (*it)->set_mysplit(_mysplit);
    }
}

void Comm::set_rcbnew(const int _rcbnew)
{
    rcbnew = _rcbnew;
    if (!subcomms_.empty())
    {
        std::list<Comm*>::iterator it;
        for (it = subcomms_.begin(); it != subcomms_.end(); it++)
            (*it)->set_rcbnew(_rcbnew);
    }
}

void Comm::set_rcbcutfrac(const double _rcbcutfrac)
{
    rcbcutfrac = _rcbcutfrac;
    if (!subcomms_.empty())
    {
        std::list<Comm*>::iterator it;
        for (it = subcomms_.begin(); it != subcomms_.end(); it++)
            (*it)->set_rcbcutfrac(_rcbcutfrac);
    }
}

void Comm::set_rcbcutdim(const int _rcbcutdim)
{
    rcbcutdim = _rcbcutdim;
    if (!subcomms_.empty())
    {
        std::list<Comm*>::iterator it;
        for (it = subcomms_.begin(); it != subcomms_.end(); it++)
            (*it)->set_rcbcutdim(_rcbcutdim);
    }
}

void Comm::set_sizes(const int _size_border, const int _size_forward, const int _size_reverse, const int _maxforward, const int _maxreverse)
{
    size_border = _size_border;
    size_forward = _size_forward;
    size_reverse = _size_reverse;
    maxforward = _maxforward;
    maxreverse = _maxreverse;
}

void Comm::set_layout(const int _layout)
{
    layout = _layout;
    if (!subcomms_.empty())
    {
