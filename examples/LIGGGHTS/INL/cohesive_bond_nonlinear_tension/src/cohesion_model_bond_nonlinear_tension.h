...
-------------------------------------------------------------------------
   ...
   Ahmed Hamed (Idaho National Laboratory)
   Zakia Tasnim (Clemson University)
------------------------------------------------------------------------- */
/* ----------------------------------------------------------------------
    This is the
    ...
    void surfacesClose(SurfacesCloseData & scdata, ForceData & i_forces, ForceData & j_forces)
    {
      ...
      if (tensionflag_ || compressionflag_)
      {
        ...
        if ( (tensionflag_ && displacement < -1.e-15) || (compressionflag_ && displacement > 1.e-15) )
        {
          ...
          double displacement_c1 = pow((ku_fn1-k_fn1)/(ku_fn1+kc_fn1),1.0)*displacement_max;
          double displacement_c2 = pow((ku_fn2-k_fn2)/(ku_fn2+kc_fn2),2.0)*displacement_min;
          double frcmag;

          if (displacement <= displacement_min)
          {frcmag = -k_fn2*pow(fabs(displacement),0.5);}
          else
          {
              if (displacement < displacement_c2)
              {frcmag = -(ku_fn2*pow(fabs(displacement),0.5)+(k_fn2-ku_fn2)*pow(fabs(displacement_min),0.5));}
              else
              {
                  if (displacement < 0.0)
                  {frcmag = kc_fn2*pow(fabs(displacement),0.5);}
                  else
                  {
                      if (displacement < displacement_c1)
                      {frcmag = -kc_fn1*A*displacement;}
                      else
                      {
                          if (displacement < displacement_max)
                          {frcmag = ku_fn1*A*displacement+(k_fn1-ku_fn1)*A*displacement_max;}
                          else
                          {frcmag = k_fn1*A*displacement;}
                      }
                  }
              }
          }
